...
import btree.types.Types;
import btree.types.LongType;


public class PageSerializer<K extends Comparable<K>> {
	private Types<K> keyDT;
	private LongType adressDT;
	private int m;
	private File file; 
	...
	public PageSerializer(File file, Types<K> dt, int m) throws IOException {
		this.file = file;
		this.keyDT = dt;
		this.adressDT = new LongType();
		this.m = m;
	...
	private void write(long offset, Page<K> page, boolean append) throws Exception {
		...
		BST<K, Long> st = page.asSymbolTable();
	...
	Page<K> append(BST<K, Long> st, boolean bottom) throws Exception {
	...
	Page<K> read(long offset) throws Exception {
		...
		BST<K, Long> st = new BST<>(keys, values, size);
		fileStore.close();
