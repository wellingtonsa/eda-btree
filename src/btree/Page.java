...
public class Page<K extends Comparable<K>> {
	...
	private BST<K, Long> st;
	private PageSerializer<K> serializer;
	private long offset;
	
	public Page(boolean bottom, long offset, PageSerializer<K> serializer) {
		this(new BST<>(serializer.getPageSize()), bottom, offset, serializer);
	}
	
	public Page(BST<K, Long> st, boolean bottom, long offset, PageSerializer<K> serializer) {
		this.st = st;
		this.bottom = bottom;
	...
	public Page<K> split() throws Exception {
		BST<K, Long> newST = this.st.split();	
		this.close();
	...
	public BST<K, Long> asSymbolTable() {
		return st;
	}
}
